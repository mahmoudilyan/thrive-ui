...
app.post('/App/Automation/GetAutomations.json', (req, res) => {
	const { view, loadheaders } = req.query;
	if (loadheaders) {
		const headersFile =
			view === 'folder'
				? './mock/automations-load-headers-folder.json'
				: './mock/automations-load-headers.json';
		fs.readFile(headersFile, 'utf8', (err, data) => {
			if (err) {
				res.status(500).send('Error reading data');
				return;
			}
	...
	if (view === 'folder') {
		if (req.body.folderid) {
			fs.readFile(`./mock/automations-single-folder-view.json`, 'utf8', (err, data) => {
				if (err) {
					res.status(500).send('Error reading data');
					return;
				}
			...
			fs.readFile('./mock/automations-folder-view.json', 'utf8', (err, data) => {
				if (err) {
					res.status(500).send('Error reading data');
					return;
				}
		...
		fs.readFile('./mock/automations-list-view.json', 'utf8', (err, data) => {
			if (err) {
				res.status(500).send('Error reading data');
				return;
			}
